   ...
   */
    ...
    */
         ...
         sceneListSelect.addEventListener('change', function() {
            ...
            if (selectedScene !== '0') {
                // Fetch speeches for the selected scene
                const selectedAct = actListSelect.value; // Get the selected act
                const selectedPlay = playListSelect.value; // Get the selected play
                ...
                fetch(`${url}?name=${selectedPlay}`)
                    ...
                    .then(data => {
                        ...
                        const sceneTitleElement = document.querySelector('.title');
                        const sceneDirectionElement = document.querySelector('.direction');
                        sceneTitleElement.textContent = `Scene title: ${scene.title}`;
                        sceneDirectionElement.textContent = `Stage direction: ${scene.stageDirection}`;
                    })
                    .catch(error => console.error('Error fetching speeches:', error));
            } else {
                resetSpeeches();
    
                // Reset scene title and direction
                const sceneTitleElement = document.querySelector('.title');
                const sceneDirectionElement = document.querySelector('.direction');
                sceneTitleElement.textContent = 'Scene title';
                sceneDirectionElement.textContent = 'Stage direction';
            }
        });
        ...
        function updateSpeeches(speeches) {
            ...
            speeches.forEach(speech => {
                ...
                speech.lines.forEach(line => {
                    ...
                    linesP.appendChild(lineP);
                });
                speechDiv.appendChild(speakerSpan);
                speechDiv.appendChild(linesP);
