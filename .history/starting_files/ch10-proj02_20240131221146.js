   ...
   */
    ...
    */
         ...
         filterButton.addEventListener('click', function() {
             ...
             // Iterate over each paragraph and highlight matching text
             speechParagraphs.forEach(paragraph => {
                 ...
                 // Check if the paragraph text contains the filter text
                 if (filterText !== '') {
                     const text = paragraph.textContent.trim().toLowerCase();
                     let index = text.indexOf(filterText);
                     while (index !== -1) {
                         // Wrap matching text in a span element with a yellow background
                         const span = document.createElement('span');
                         span.className = 'highlight';
                         span.style.backgroundColor = 'yellow';
                         const matchedText = paragraph.textContent.substring(index, index + filterText.length);
                         const replacement = '<span class="highlight" style="background-color: yellow;">' + matchedText + '</span>';
                         
                         // Replace the matching text with the span element
                         paragraph.innerHTML = paragraph.innerHTML.substring(0, index) +
                             replacement +
                             paragraph.innerHTML.substring(index + filterText.length);
     
                         // Continue searching for next occurrence of filter text
                         index = text.indexOf(filterText, index + 1);
                     }
                 }
             });
         });
