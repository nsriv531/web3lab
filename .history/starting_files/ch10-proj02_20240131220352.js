    ...
    filterButton.addEventListener('click', function() {
        ...
        speechParagraphs.forEach(paragraph => {
            ...
            if (filterText !== '') {
                ...
                const index = text.indexOf(filterText);
                if (index !== -1) {
                    // Highlight matching text by wrapping in a span element with inline styles
                    const highlightedText = paragraph.textContent.substring(0, index) +
                        '<span style="background-color: yellow;">' +
                        paragraph.textContent.substring(index, index + filterText.length) +
                        '</span>' +
