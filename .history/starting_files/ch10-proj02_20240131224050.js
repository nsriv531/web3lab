document.addEventListener("DOMContentLoaded", function() {	
   ...
   */
    ...
    */
         ...
         const filterButton = document.getElementById('btnHighlight');
         filterButton.addEventListener('click', function() {
            // Get the text entered in the filter input
            const filterText = document.getElementById('txtHighlight').value.trim().toLowerCase();
        
            // Get all speech paragraphs
            const speechParagraphs = document.querySelectorAll('.speech p');
        
            // Iterate over each paragraph
            speechParagraphs.forEach(paragraph => {
                const text = paragraph.textContent.trim().toLowerCase();
                const highlightedText = [];
        
                // Clear existing highlights
                paragraph.querySelectorAll('.highlight').forEach(span => {
                ...
                if (filterText !== '') {
                    let startIndex = 0;
                    let index;
                    while ((index = text.indexOf(filterText, startIndex)) !== -1) {
                        // Check if the match is a whole word
                        const isWholeWord = isWholeWordMatch(text, filterText, index);
                
                        if (isWholeWord) {
                            const range = document.createRange();
                            const startNode = paragraph.firstChild;
                            const startOffset = Math.min(index, startNode.textContent.length - 1); // Ensure offset is within valid range
                        
                            if (startOffset >= 0) { // Check if the index is valid
                                const endOffset = startOffset + filterText.length;
                                range.setStart(startNode, startOffset);
                                range.setEnd(startNode, endOffset);
                                const span = document.createElement('span');
                                span.className = 'highlight';
                                span.style.backgroundColor = 'yellow'; // Ensure the background color is set to yellow
                                range.surroundContents(span);
                                highlightedText.push(filterText);
                ...
                // Show/hide paragraph based on filter text
                if (filterText === '' || highlightedText.length > 0) {
...
function isWholeWordMatch(text, filterText, index) {
    const beforeChar = index === 0 || !isAlphaNumeric(text[index - 1]);
    const afterChar = index + filterText.length === text.length || !isAlphaNumeric(text[index + filterText.length]);
