    ...
    */
         ...
         filterButton.addEventListener('click', function() {
            ...
            filterButton.addEventListener('click', function() {
                ...
                // Iterate over each paragraph and show/hide based on the filter text
                speechParagraphs.forEach(paragraph => {
                    ...
                    if (filterText !== '') {
                        ...
                        let index; // Declare index here
            
                        while ((index = textContent.toLowerCase().indexOf(filterText, startIndex)) > -1) {
                            // Create a range to select the matched text
                            const range = document.createRange();
                            
                            // Ensure that the calculated offset is within the valid range
                            const offset = Math.min(index, textContent.length - 1);
                            
                            range.setStart(paragraph.firstChild, offset);
                            range.setEnd(paragraph.firstChild, offset + filterText.length);
            
                            // Wrap the range in a <span> element with the highlight class
                            const span = document.createElement('span');
                            span.className = 'highlight';
                            span.style.backgroundColor = 'yellow';
                            range.surroundContents(span);  
                            startIndex = index + filterText.length;
                        }
                    }
                });
            });
